...
- WiFi 配网：支持首次 SoftAP 配网，以及 WiFi 1 / WiFi 2 两组接入配置；正常连接后可通过 IP 或 `sms.local` 访问。
- 账号密码：Web 管理账号可单独保存。
- 时间设置：NTP 服务器和时区可配置。
...
- 支持 SoftAP 配网、WiFi 1 / WiFi 2 主备接入、`sms.local` mDNS、NTP 时间同步、AT 终端和运行状态查看。
...
首次启动时设备会优先连接已保存 WiFi；Web UI 可保存 WiFi 1 和 WiFi 2 两组网络，设备会先尝试 WiFi 1，失败后尝试 WiFi 2。如果没有配置或两组网络都连接失败，会开启开放热点 `SMS-Forwarder-XXXX`。连接热点后访问 `http://192.168.1.1` 配网；设备连上路由器后，可通过串口日志里的 IP 地址或 `http://sms.local` 打开 Web UI。
...
出厂 seed 只作为 WiFi 1 的初始兜底配置；第二组备用 WiFi 可在 Web UI 的“WiFi 网络”里保存。也可以留空，完全走 Web 配网。
